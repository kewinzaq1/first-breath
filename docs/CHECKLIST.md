...
5. **Claude Code with Bright Data:** open a fresh Claude Code session in `~/Documents/first-breath`, type `/mcp` → `brightdata` connected. Run prompt 1 from `docs/DEMO.md` once (if it says "Unexpected non-JSON response", wait 15 s and ask again). Leave the session open behind Brave. Fallback: `docs/stage/chat-outputs.md`.
6. **The page is the demo:** open the artifact URL in Brave, full screen, scrolled to scene 1 (the orb). Run-of-show: `docs/DEMO.md` (page 2:20 → chat 1:40 → the minute 1:00). In the last scene `Enter` starts the Moment, `Escape` aborts. (`docs/show/index.html` + `docs/SHOW.md` and the PowerPoint deck are alternates, not tonight's plan.)
...
10. Water. First line: *"I built a meditation app called Moment. Before telling anyone, I asked the web whether the idea was true. This page is the verdict."*
