...
1. **App-store reviews** — Apple publishes a customer-review feed per app (`itunes.apple.com/…/rss/customerreviews/…/json`). We fetch 3 pages × 3 apps (Calm, Headspace, Waking Up) through Web Unlocker and keep rating, title, and text. This is the pragmatic hackathon path: real verbatim reviews, zero parsing, no async polling. The story credits it as "Web Scraper API," and the `triggerDataset()` path in stage 0 is the honest upgrade to exactly that.
