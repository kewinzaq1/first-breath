...
**Pain → proof → fix → how.** Meditation apps run the social-feed playbook (keep you coming back, give you more, charge you more) — that is Kew's *claim*. The research exists to turn the claim into *proof* (48% paywall fatigue, verbatim rage, "$70 for a timer", a search gap with no timer in it). The *fix* is subtraction: one timer, counted breaths, nothing else. Bright Data is the *how*, told last. The page and the deck (`docs/first-breath-how-it-works.pptx`, generated by `docs/deck.js`) both follow this order; keep it when editing either.
